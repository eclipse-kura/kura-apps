...
public class BluetoothLe implements ConfigurableComponent {
    ...
    private void filterDevices(List<BluetoothLeDevice> devices) {
        ...
        for (BluetoothLeDevice bluetoothLeDevice : devices) {
            if (bluetoothLeDevice.getAddress() == null || bluetoothLeDevice.getAlias() == null) {
                continue;
            }

            logger.info("Address {} Name {}", bluetoothLeDevice.getAddress(), bluetoothLeDevice.getAlias());

            if (bluetoothLeDevice.getAlias().replace(" ", "").contains("SensorTag")
